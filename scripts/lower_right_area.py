...
from scripts.date_verification import *
from scripts.cryptocurrencies_prices import *
from scripts.scrolling_menus import Menu
from scripts.find_peaks import find_peaks
...
class InformationBoxes:
    ...
    def __init__(self, root, values: list, crypto: str, physical_currency: str) -> None:
        ...
        self.symbol = CRYPTOCURRENCIES_SYMBOLS[self.physical_currency]
    ...
    def day_information(self) -> None:
        ...
        date_valid = is_date_valid(day_entry, self.crypto == 'SHIB')
        ...
        if date_valid[0]:
            ...
            if date_valid[1]:
                number_of_days = (date.today() - date(day=30, month=9, year=2021)).days
                self.day_day_error_label['text'] = "not valid, must be an DD/MM/YYYY date\n" \
                                                   f"between {number_of_days} days ago and yesterday"
            else:
                self.day_day_error_label['text'] = "not valid, must be an DD/MM/YYYY date\n" \
